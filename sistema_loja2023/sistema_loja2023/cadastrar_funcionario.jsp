   ...
   <body>
      <form name="form1" method="post" action="funcionario.jsp">
         ...
         <table border=1 cellspacing=0 cellpadding=0>
            ...
            <% try { con.setFun_codigo(Integer.parseInt(request.getParameter("fun_codigoField")));
               ...
               con.setFun_salario(request.getParameter("fun_salarioField")); con.inserirDados(); con.setConsulta();
               ResultSet temp=con.getResultado(); temp.next();
               if(request.getParameter("fun_codigoField").equals(temp.getString("fun_codigo")))
               response.sendRedirect("http://localhost:8084/sistema_loja2023/funcionario.jsp?status=Registro ja existente"); do{ %>
               <tr>
                  <td width=50>
         ...
         <p><strong>Status:
               <input name="statusField" disabled type="text" id="statusField" size="36"
                  value="<%=request.getParameter(" fun_codigoField")%> foi cadastrado com sucesso">
            </strong></p>
      </form>
   </body>
