   ...
   <body>
      <form name="form1" method="post" action="tipopagto.jsp">
         ...
         <table border=1 cellspacing=0 cellpadding=0>
            ...
            <% try { con.setTpg_codigo(Integer.parseInt(request.getParameter("tpg_codigoField")));
               con.setTpg_descricao(request.getParameter("tpg_descricaoField"));
               con.setTpg_qtde(request.getParameter("tpg_qtdeField"));
               con.setTpg_ativo(request.getParameter("tpg_ativoField")); con.inserirDados(); con.setConsulta();
               ResultSet temp=con.getResultado(); temp.next();
               if(request.getParameter("tpg_codigoField").equals(temp.getString("tpg_codigo")))
               response.sendRedirect("http://localhost:8084/sistema_loja2023/tipopagto.jsp?status=Registro ja existente"); do{ %>
               <tr>
                  <td width=50>
         ...
         <p><strong>Status:
               <input name="statusField" disabled type="text" id="statusField" size="36"
                  value="<%=request.getParameter(" tpg_codigoField")%> foi cadastrado com sucesso">
            </strong></p>
      </form>
   </body>
