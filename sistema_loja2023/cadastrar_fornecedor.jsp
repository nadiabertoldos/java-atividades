   ...
   <body>
      <form name="form1" method="post" action="fornecedor.jsp">
         ...
         <table border=1 cellspacing=0 cellpadding=0>
            ...
            <% try { con.setFor_codigo(Integer.parseInt(request.getParameter("for_codigoField")));
               ...
               con.setFor_email(request.getParameter("for_emailField")); con.inserirDados(); con.setConsulta();
               ResultSet temp=con.getResultado(); temp.next();
               if(request.getParameter("for_codigoField").equals(temp.getString("for_codigo")))
               response.sendRedirect("http://localhost:8084/sistema_loja2023/fornecedor.jsp?status=Registro ja existente"); do{ %>
               <tr>
                  <td width=50>
         ...
         <p><strong>Status:
               <input name="statusField" disabled type="text" id="statusField" size="36"
                  value="<%=request.getParameter(" for_codigoField")%> foi cadastrado com sucesso">
            </strong></p>
      </form>
   </body>
