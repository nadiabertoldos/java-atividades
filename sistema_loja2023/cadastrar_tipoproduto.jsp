   ...
   <body>
      <form name="form1" method="post" action="tipoproduto.jsp">
         ...
         <table border=1 cellspacing=0 cellpadding=0>
            ...
            <% try { con.setTpp_codigo(Integer.parseInt(request.getParameter("tpp_codigoField")));
               con.setTpp_descricao(request.getParameter("tpp_descricaoField")); con.inserirDados(); con.setConsulta();
               ResultSet temp=con.getResultado(); temp.next();
               if(request.getParameter("tpp_codigoField").equals(temp.getString("tpp_codigo")))
               response.sendRedirect("http://localhost:8084/sistema_loja2023/tipoproduto.jsp?status=Registro ja existente"); do{ %>
               <tr>
                  <td width=50>
         ...
         <p><strong>Status:
               <input name="statusField" disabled type="text" id="statusField" size="36"
                  value="<%=Integer.parseInt(request.getParameter(" tpp_codigoField"))%> foi cadastrado com sucesso">
            </strong></p>
      </form>
   </body>
